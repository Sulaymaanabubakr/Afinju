      ...
      <section className="bg-afinju-black text-afinju-cream py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="font-body text-afinju-cream/55">
            Afínjú is crafted to your exact dimensions. Taking accurate measurements ensures your authority set fits perfectly.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto space-y-20">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-10">
              <div className="space-y-5 font-body text-afinju-black/65 leading-relaxed text-center">
                  ...
                  <thead className="bg-afinju-cream">
                    <tr>
                      {['EU', 'UK', 'US', 'Foot Length (cm)'].map(h => (
                        <th key={h} className="px-4 py-3 text-center font-sans text-[10px] tracking-[0.15em] uppercase text-afinju-black/50">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-black/5">
                    {shoeData.map(row => (
                      <tr key={row.eu} className="hover:bg-afinju-cream/40 transition-colors">
                        <td className="px-4 py-3 text-center font-sans text-sm font-semibold text-gold">{row.eu}</td>
                        <td className="px-4 py-3 text-center font-sans text-sm">{row.uk}</td>
                        <td className="px-4 py-3 text-center font-sans text-sm">{row.us}</td>
                        <td className="px-4 py-3 text-center font-sans text-sm">{row.cm} cm</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="divide-y divide-black/5">
                    {headData.map(row => (
                      <tr key={row.cm} className="hover:bg-afinju-cream/40 transition-colors">
                        <td className="px-4 py-3 text-center font-sans text-sm font-semibold text-gold">{row.cm} cm</td>
                        <td className="px-4 py-3 text-center font-sans text-sm">{row.inches}"</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="bg-afinju-cream p-8 text-center">
            ...
            <a
              href={`https://wa.me/2347071861932?text=${encodeURIComponent('Hello Afínjú, I need help measuring my size before ordering.')}`}
              target="_blank"
